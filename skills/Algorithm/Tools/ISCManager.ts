...
EXAMPLES:
  ...
  bun run ISCManager.ts add -d "Toggle component works" -s EXPLICIT --verify-method browser --verify-criteria "Toggle visible"
...
`);
  ...
  switch (command) {
    ...
    case "claim": {
      ...
      if (!values.row || !values.agent) {
        console.error("Error: --row and --agent are required for claim");
        process.exit(1);
      }
    ...
    case "research-block": {
      ...
      if (!values.row || !values.reason) {
        console.error("Error: --row and --reason are required for research-block");
        process.exit(1);
      }
    ...
    case "acknowledge": {
      ...
      if (!values.row || !values.action) {
        console.error("Error: --row and --action are required for acknowledge");
        process.exit(1);
      }
    ...
    case "child-status": {
      ...
      if (!values.row || !values["child-status"]) {
        console.error("Error: --row and --child-status are required");
        process.exit(1);
      }
