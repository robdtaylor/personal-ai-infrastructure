...
**IMPORTANT:** These paths are where key tools are ACTUALLY installed on this system. Use these paths in launchd plists, cron jobs, and scripts.

| Tool | Path | Notes |
|------|------|-------|
| **bun** | `/opt/homebrew/bin/bun` | NOT ~/.bun/bin/bun - installed via Homebrew |
| **node** | `/opt/homebrew/bin/node` | Homebrew installation |
| **python3** | `/opt/homebrew/bin/python3` | Homebrew installation |
| **claude** | `/Users/robt/.claude/local/claude` | Claude Code CLI |

**Why this matters:** When creating launchd agents or cron jobs, they run in a minimal environment. Always use absolute paths to binaries, and verify the path exists before deploying.

```bash
# Verify a binary path
which bun  # Returns actual location
```
