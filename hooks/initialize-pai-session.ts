...
async function sendNotification(title: string, message: string, priority: string = 'normal') {
  try {
    // Use kai voice ID (customize in settings.json if needed)
    const voiceId = process.env.DA_VOICE_ID || 'kai';
...
async function testStopHook() {
  ...
  try {
    ...
    // Set terminal background color to #2A301E
    process.stderr.write('\x1b]11;#2A301E\x07');
...
async function main() {
  try {
    ...
    await sendNotification('Session Started', message, 'normal');
    console.error(`🔊 Startup notification sent: "${message}"`);
